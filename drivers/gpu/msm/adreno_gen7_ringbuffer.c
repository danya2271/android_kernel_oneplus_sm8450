...
// Maximum dwords for the fixed (non-IB list) commands generated by gen7_ringbuffer_submitcmd
#define GEN7_SUBMITCMD_MAX_FIXED_DWORDS 24
// Maximum dwords consumed by a single IB entry (either a real IB or a NOP for a skipped one)
#define GEN7_SUBMITCMD_DWORDS_PER_IB_ENTRY 5
// Threshold for the number of IB entries; if <= this, stack allocation is used.
#define GEN7_SUBMITCMD_MAX_IB_ENTRIES_FOR_STACK 32

// Calculate the total maximum dwords needed for the stack-allocated buffer
#define GEN7_SUBMITCMD_STACK_ALLOC_MAX_DWORDS \
	(GEN7_SUBMITCMD_MAX_FIXED_DWORDS + \
	 (GEN7_SUBMITCMD_MAX_IB_ENTRIES_FOR_STACK * GEN7_SUBMITCMD_DWORDS_PER_IB_ENTRY))
		...
		struct adreno_submit_time *time)
{
	...
	int ret = 0, index = 0;
	bool concurrent_binning = is_concurrent_binning(drawctxt);

	u32 *cmds_ptr;
	// Declare stack buffer using the pre-calculated max size
	u32 stack_cmds_buffer[GEN7_SUBMITCMD_STACK_ALLOC_MAX_DWORDS];
	bool used_kvmalloc = false;
	unsigned int num_ib_entries = 0;
	unsigned int required_dwords_for_alloc;

	/* 1. Count IB entries if not skipping the whole submission */
	if (!IS_SKIP(flags)) {
		struct list_head *tmp_ib_node;
		list_for_each(tmp_ib_node, &cmdobj->cmdlist)
			num_ib_entries++;
	}

	/* 2. Determine allocation strategy and buffer pointer */
	if (likely(num_ib_entries <= GEN7_SUBMITCMD_MAX_IB_ENTRIES_FOR_STACK)) {
		// Common case: Use stack allocation.
		// GEN7_SUBMITCMD_STACK_ALLOC_MAX_DWORDS is guaranteed to be large enough.
		cmds_ptr = stack_cmds_buffer;
	} else {
		// Less common case: Too many IBs for stack, fall back to kvmalloc.
		// Calculate precise dwords needed for fixed commands:
		unsigned int actual_fixed_dwords = 2; // CP_NOP, START_IB_IDENTIFIER

		if (concurrent_binning)
			actual_fixed_dwords += 2; // CP_THREAD_CONTROL
		if (num_ib_entries > 0)
			actual_fixed_dwords += 4; // CP_SET_MARKER (start/end of IB list)
		if (concurrent_binning)
			actual_fixed_dwords += 2; // CP_THREAD_CONTROL
		actual_fixed_dwords += 2;         // CCU invalidate depth
		actual_fixed_dwords += 2;         // CCU invalidate color
		actual_fixed_dwords += 2;         // CP_NOP, END_IB_IDENTIFIER

		required_dwords_for_alloc = actual_fixed_dwords +
			(num_ib_entries * GEN7_SUBMITCMD_DWORDS_PER_IB_ENTRY);

		cmds_ptr = kvmalloc(required_dwords_for_alloc * sizeof(u32), GFP_KERNEL);
		if (!cmds_ptr)
			return -ENOMEM;
		used_kvmalloc = true;
	}

	/* 3. Populate the command buffer (cmds_ptr) */
	// index is reset/initialized to 0 implicitly by being a local variable,
	// or explicitly index = 0; if it were static or reused.

	cmds_ptr[index++] = cp_type7_packet(CP_NOP, 1);
	cmds_ptr[index++] = START_IB_IDENTIFIER;

	if (concurrent_binning) {
		cmds_ptr[index++] = cp_type7_packet(CP_THREAD_CONTROL, 1);
		cmds_ptr[index++] = CP_SET_THREAD_BOTH;
	}

	if (num_ib_entries > 0) {
		struct kgsl_memobj_node *ib;

		cmds_ptr[index++] = cp_type7_packet(CP_SET_MARKER, 1);
		cmds_ptr[index++] = 0x00d; /* IB1LIST start */

		list_for_each_entry(ib, &cmdobj->cmdlist, node) {
			if (ib->priv & MEMOBJ_SKIP ||
				(ib->flags & KGSL_CMDLIST_CTXTSWITCH_PREAMBLE &&
				 !IS_PREAMBLE(flags))) {
				cmds_ptr[index++] = cp_type7_packet(CP_NOP, 4);
				cmds_ptr[index++] = 0;
				cmds_ptr[index++] = 0;
				cmds_ptr[index++] = 0;
				cmds_ptr[index++] = 0;
				...
				cmds_ptr[index++] = lower_32_bits(ib->gpuaddr);
				cmds_ptr[index++] = upper_32_bits(ib->gpuaddr);
				cmds_ptr[index++] = (ib->size >> 2) & 0xfffff;
			}
		}

		cmds_ptr[index++] = cp_type7_packet(CP_SET_MARKER, 1);
		cmds_ptr[index++] = 0x00e; /* IB1LIST end */
	}

	if (concurrent_binning) {
		cmds_ptr[index++] = cp_type7_packet(CP_THREAD_CONTROL, 1);
		cmds_ptr[index++] = CP_SET_THREAD_BR;
	}
	cmds_ptr[index++] = cp_type7_packet(CP_EVENT_WRITE, 1);
	cmds_ptr[index++] = 24; // CCU invalidate depth

	cmds_ptr[index++] = cp_type7_packet(CP_EVENT_WRITE, 1);
	cmds_ptr[index++] = 25; // CCU invalidate color

	cmds_ptr[index++] = cp_type7_packet(CP_NOP, 1);
	cmds_ptr[index++] = END_IB_IDENTIFIER;

	/*
	 * 'index' now holds the actual number of dwords written.
	 * For stack path, index <= GEN7_SUBMITCMD_STACK_ALLOC_MAX_DWORDS.
	 * For kvmalloc path, index <= required_dwords_for_alloc (potentially smaller if some IBs took 4 dwords).
	 */
	...
	/* Pass 'index' as the actual number of dwords used */
	ret = gen7_ringbuffer_addcmds(adreno_dev, drawctxt->rb, drawctxt,
		flags, cmds_ptr, index, drawobj->timestamp, time);
