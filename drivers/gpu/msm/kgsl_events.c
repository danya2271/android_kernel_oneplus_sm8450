		...
		struct kgsl_event_group *group)
{
	...
	spin_lock_irqsave(&device->event_groups_lock, flags);
	list_del_rcu(&group->group);
	spin_unlock_irqrestore(&device->event_groups_lock, flags);
	synchronize_rcu();
}
...
void kgsl_device_events_remove(struct kgsl_device *device)
{
	...
	/* Wait for all RCU readers to finish before the device (and its groups) are freed */
	synchronize_rcu();
}
