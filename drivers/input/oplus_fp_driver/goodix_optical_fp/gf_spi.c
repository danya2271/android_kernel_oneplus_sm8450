...
#if IS_ENABLED(CONFIG_DRM_PANEL_NOTIFY_FINGERPRINT)
#ifndef PANEL_EVENT_NOTIFICATION_PRIMARY
...
fp_panel_notifier_callback(enum panel_event_notifier_tag tag,
			   ...
			   void *client_data)
{
	struct gf_dev *gf_dev = (struct gf_dev *)client_data;
	char msg = 0;
	...
	case (int)DRM_PANEL_EVENT_UNBLANK:
		if (gf_dev->device_available == 1) {
			pr_debug("%s DRM_PANEL_EVENT_UNBLANK enter\n", __func__);
		}
		break;
	case (int)DRM_PANEL_EVENT_BLANK:
		if (gf_dev->device_available == 1) {
			pr_debug("%s DRM_PANEL_EVENT_BLANK enter\n", __func__);
		}
		break;
	case (int)DRM_PANEL_EVENT_BLANK_LP:
		pr_debug("%s DRM_PANEL_EVENT_BLANK_LP enter\n", __func__);
		break;
	case (int)DRM_PANEL_EVENT_FPS_CHANGE:
		pr_debug("%s DRM_PANEL_EVENT_FPS_CHANGE enter\n", __func__);
		break;
...
static int fp_check_panel_dt(struct gf_dev *gf_dev)
{
	...
	for (i = 0; i < count; i++) {
		...
		of_node_put(node);
		if (!IS_ERR(panel)) {
			pr_info("%s, active_panel = %p\n", __func__, panel);
			gf_dev->active_panel = panel;
			return 0;
		}
	}

	pr_info("%s, NO active panel!\n", __func__);
